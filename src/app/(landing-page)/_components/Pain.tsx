...
const PainSolutionSection = () => {
  return (
    <section className="py-2 bg-[#fdf9f3]">
      {painSolutionData.map((item, idx) => (
        <motion.div
          key={idx}
          className={`flex flex-col md:flex-row ${
            idx % 2 ? "md:flex-row-reverse" : ""
          } items-center py-9 px-8 lg:px-13 gap-13
                      `}
        ...
        >
          ...
          <div className="md:w-1/2 space-y-4">
            ...
            <p className="text-sm text-[#5c4a38]/90 italic">{item.problem}</p>
            <p className="text-sm text-[#5c4a38] font-medium">{item.promise}</p>
            <p className="text-sm text-[#5c4a38]">{item.payoff}</p>
          </div>
