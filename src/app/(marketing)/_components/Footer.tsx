...
const Footer = () => {
  ...
  return (
    <footer className="py-10 px-10">
      <div className="max-w-360 mx-auto rounded-3xl bg-surface-raised border border-line shadow-e2">
        <div className="flex flex-col space-y-6 md:space-y-0 items-center md:flex-row md:justify-between px-8 pt-7">
          ...
          <div className="flex items-center">
            ...
            <Link
              href="/"
              className="text-ink font-sora text-2xl font-semibold tracking-tight -ml-2 pt-1"
            ...
            <nav className="flex flex-wrap justify-center gap-6 text-sm font-sora">
              {footerNavLinks.map(({ href, key }, idx: number) => (
                <Link
                  key={idx}
                  href={href}
                  className={`link-animation ${pathName === href ? "text-accent-text font-medium" : "text-ink-muted"}`}
                >
                  {t(key)}
                </Link>
              ))}
        ...
        <div className="px-8 pb-7">
          <p className="text-xs text-ink-subtle text-center font-sora py-1">
